...
**Imagen PNG:** 
![01-solid-05-dip.png](/diagramas/01-diagrama-clases/01-solid-05-dip.png)


## Justificación Técnica:
Sobre el diagrama solid-05-dip podemos observar:

**PARA LA CLASE ServicioNotificaciones:** Un módulo de alto nivel `ServicioNotificaciones` que tiene dependencias internas del tipo interfaz (`ICanalNotificacion` y `IRepoNotificacion`), el cual recibe abstracciones por inyección, no depende de estructuras de bajo nivel (implementaciones), las cuales pueden ser intercambiables.
...
**PARA LA CLASE ETAPA:** Cuando cambia de estado, prepara el contenido del aviso (regla de negocio) y delegá el “cómo” a `INotificadorService`. De esta forma no queda atada a `CanalEmail`, `CanalWhatsApp` ni a ningún detalle de persistencia.
→ Beneficio: bajo acoplamiento, test simple con mocks y posibilidad de agregar canales sin tocar la clase.

**PARA LA CLASE USUARIO:** Al notificar, define el “qué” comunicar (asunto y mensaje) y también delegá el “cómo” en `INotificadorService`. Su `canalPreferido` puede usarse como política dentro del servicio o en la composición, sin acoplar la clase a implementaciones concretas.
→ Beneficio: separación de responsabilidades, pruebas sin efectos externos y facilidad para cambiar o sumar transportes/almacenamientos.